...
public class MaterialWood extends Material {
    ...
    public MaterialWood() {
        super(4, 2000, true, MaterialType.WALL, "Log");
    }
    ...
    public void destroy(World world, float x, float y) {
        ...
        outer: for(int i = 1; i < 50; i++) {
            Block block = world.getWall(blockX, blockY - (i * Material.SIZE));
            ...
            if(block.getMaterial().getID() == Material.WOOD.getID()) {
                ...
                for(int j = 0; j < 3; j++) {
                    for(int k = 0; k < 3; k++) {
                        Block leafblock = world.getWall(blockX + (j * Material.SIZE), blockY + (k * Material.SIZE));
